...
<html>
<head>
    <title>Update Employee - Gym Management System</title>
...
<body>
	...
	try {
	    ...
	    String update_empl_sql =
	    "UPDATE employees SET admin_flag = ?, coach_flag = ? "
	    + "WHERE user_id = ?";
	    PreparedStatement update_empl_stmt =
	    con.prepareStatement(update_empl_sql);
	    update_empl_stmt.setInt(1, (admin_flag != null) ? 1 : 0);
	    update_empl_stmt.setInt(2, (coach_flag != null) ? 1 : 0);
	    update_empl_stmt.setInt(3, update_user_id);
	    update_empl_stmt.executeUpdate();
	    update_empl_stmt.close();
    %>
    <div class="card container-input">
	<form action="users.jsp" method="get">
	    <div class="form-row">
		<label>Employee updated successfully</label>
	    </div>
	    <div class="form-row">
	...
	<form action="update-employee.jsp" method="post">
	    ...
	    <div class="form-row">
	    <label>Username</label>
	    <input id="username" name="username" type="text" value="<%= empRs.getString("u.user_name") %>" />
	</div>
	<div class="form-row">
	    <label>Password</label>
	...
	<div class="form-row">
	    <label>First name</label>
	    <input id="fname" name="fname" type="text" value="<%= empRs.getString("u.first_name") %>" />
	    <label>Last name</label>
	    <input id="lname" name="lname" type="text" value="<%= empRs.getString("u.last_name") %>" />
	</div>
	<div class="form-row">
	    <input id="coach_flag" name="coach_flag" type="checkbox" value="coach_flag" <% if (empRs.getInt("e.coach_flag") == 1) out.print("checked"); %> />
	    <label class="check">Coach</label>
	    <input id="admin_flag" name="admin_flag" type="checkbox" value="admin_flag" <% if (empRs.getInt("e.admin_flag") == 1) out.print("checked"); %> />
	    <label class="check">Administrator</label>
	    <input id="active_flag" name="active_flag" type="checkbox" value="active_flag" <% if (empRs.getInt("u.active_flag") == 1) out.print("checked"); %> />
	    <label class="check">Active</label>
	</div>
	<button type="submit" class="btn" style="width: 100%;">Update</button>
	</form>
